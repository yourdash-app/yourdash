...
export default class WebsocketManager {
  private openSocketConnections: Map<
    `${string}-${number | string}` /* `[username]-[sessionId]` */,
    WebsocketManagerServerConnection
  > = new Map();
  ...
  constructor(rootPath: string) {
    ...
    this.socketIoServer.on("connection", (socket: socketIo.Socket<any, any, any>) => {
      ...
      const session = coreApi.users
        .get(handshakeUsername)
        .getLoginSessionByToken(socket.handshake.query.sessionToken as string);

      if (session.type !== YOURDASH_SESSION_TYPE.desktop) return socket.disconnect(true);
      ...
      socket.on("disconnect", () => {
        this.__internal__removeSocketConnection(
          this.openSocketConnections.get(`${handshakeUsername}-${socket.handshake.query.sessionId}`),
        );
    ...
    this.socketIoServer.use(async (socket: socketIo.Socket, next) => {
      ...
      if (
        coreApi.users
          ...
          [username].find((session) => session.sessionToken === sessionToken)
      ) {
        return next();
      }
