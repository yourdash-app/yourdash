...
function applicationStartup() {
  ...
  if (ENV.DevMode) log( `(Start up) EnvironmentVariable Dev detected as: ${ ENV.DevMode }` );
  ...
  switch (true) {
    ...
    case !(SERVER_CONFIG?.loginPageConfig?.logo?.position instanceof Object):
      ...
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.left === "string" ||
      ...
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.top === "string" ||
      ...
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.right === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.right === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.logo.position.right' property!"
      );
      process.exit( 1 );
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.bottom === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.logo?.position?.bottom === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.logo.position.bottom' property!"
      );
      process.exit( 1 );
      break;
    ...
    case !(SERVER_CONFIG?.loginPageConfig?.message?.position instanceof Object):
      ...
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.left === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.left === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.message.position.left' property!"
      );
      process.exit( 1 );
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.top === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.top === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.message.position.top' property!"
      );
      process.exit( 1 );
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.right === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.right === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.message.position.right' property!"
      );
      process.exit( 1 );
      break;
    case !(
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.bottom === "string" ||
        typeof SERVER_CONFIG?.loginPageConfig?.message?.position?.bottom === typeof null
    ):
      log(
          "(Start up) ERROR: yourdash.config.json is missing the 'loginPageConfig.message.position.bottom' property!"
      );
      process.exit( 1 );
      break;
    case !SERVER_CONFIG.activeModules.includes( "core" ):
      console.error(
          chalk.redBright( `(Start up) ERROR: the 'core' module is not enabled in yourdash.config.json` )
      );
      process.exit( 1 );
      break;
    case !SERVER_CONFIG.activeModules.includes( "userManagement" ):
      console.error(
          chalk.redBright( `(Start up) ERROR: the 'userManagement' module is not enabled in yourdash.config.json` )
      );
      process.exit( 1 );
      break;
  ...
  }, 1200000 /* every 20minutes */ );

  // Allow loading cached images from the server
  ...
  modulesToLoad.forEach( (moduleName) => {
    if (!fs.existsSync( path.resolve( `./modules/${ moduleName }/index.js` ) ))
      return log(
          `(Start up) no such module: ${ moduleName }, non-existent modules should not be listed in the activeModules found in yourdash.config.json`
      );
  ...
  app.use( (req, res, next) => {
    ...
    switch (req.method) {
      case "GET":
        ...
        if (JSON.stringify( req.query ) !== "{}") log( JSON.stringify( req.query ) );
        break;
      case "POST":
        log(
            `${ date.getHours() }:${ date.getMinutes() }:${
      ...
      case "DELETE":
        ...
        // case 'OPTIONS':
        // log(
        //     `${date.getHours()}:${date.getMinutes()}:${date.getSeconds() < 10 ? `${date.getSeconds()}0` :
        // date.getSeconds() } ${chalk.bgMagenta(chalk.whiteBright(' OPT '))} ${res.statusCode} ${req.path}` ) if
        // (JSON.stringify(req.query) !== "{}") log(JSON.stringify(req.query)) break;
    }
    next();
  } );
  ...
  if (ENV.DevMode) {
    ...
    fs.readdir( `/etc/letsencrypt/live`, (err, files) => {
      ...
      if (files[0] === "README") indToRead = 1;

      fs.readFile( `/etc/letsencrypt/live/${ files[indToRead] }/privkey.pem`, (err, data) => {
        if (err) {
          log(
              `(Start up) CRITICAL ERROR: /etc/letsencrypt/live/${ files[1] }/privkey.pem not found or couldn't be read, terminating server software`
          );
          return process.exit( 1 );
        }
        ...
        fs.readFile( `/etc/letsencrypt/live/${ files[indToRead] }/fullchain.pem`, (err, data) => {
          if (err) {
            log(
                `(Start up) CRITICAL ERROR: CRITICAL ERROR: /etc/letsencrypt/live/${ files[indToRead] }/fullchain.pem not found or couldn't be read, terminating server software`
            );
            return process.exit( 1 );
          }
