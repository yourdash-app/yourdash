...
const StatusIndicators: React.FC = () => {
  ...
  useEffect( () => {
    const socket = io(
      `${csi.getInstanceUrl().replace( "https://", "wss://" ).replace( "http://", "ws://" )}`, {
  ...
  return <div className={ "ml-auto flex gap-2 items-center" }>
    <Indicator
    ...
    />
    <Indicator
      icon={YourDashIcon.Alert}
      color={"#eab842"}
      displayName={"Warning Logs"}
      value={warningLogs}
    />
    <Indicator
      icon={YourDashIcon.XCircle}
      color={"#ff6633"}
      displayName={"Error Logs"}
      value={errorLogs}
    />
    <Indicator
      icon={YourDashIcon.Bug}
      color={"#0ac700"}
