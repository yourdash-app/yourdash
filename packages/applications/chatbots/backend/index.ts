...
import { LOG_TYPE } from "backend/src/core/coreApiLog.js";
import BackendModule, {
  YourDashModuleArguments,
} from "backend/src/core/moduleManager/backendModule.js";
...
export default class DiffusionLabModule extends BackendModule {
  constructor(args: YourDashModuleArguments) {
    ...
    this.API.request.post(
      "/app/chatbots/integration/discord/authorize-user",
      async (req, res) => {
        const user = this.API.getUser(req);

        try {
          await (
            await coreApi.fs.getOrCreateFile(
              path.join(user.path, "apps/chatbots/discord/key.json"),
            )
          ).write(
            JSON.stringify({
  ...
  async getBotOwnerToken(req: ExpressRequest): Promise<string> {
    ...
    return (
      await (
        await coreApi.fs.getFile(
          path.join(user.path, "fs/apps/chatbots/discord/key.json"),
        )
      ).read("json")
    ).botOwnerToken;
  }
