...
const InstanceInfo: React.FC = () => {
  const [metadata, setMetadata] = useState<{ title: string, message: string }>( { title: "", message: "" } )
  
  useEffect( () => {
    csi.getJson( "/core/login/instance/metadata", ( json: { title: string, message: string } ) => {
      setMetadata( json );
    } );
  }, [] )
