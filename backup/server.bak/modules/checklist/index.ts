...
import { type TasksList } from "types/checklist/list.js";
...
const module: YourDashModule = {
    ...
    load(request, moduleApi) {
        request.post(`/personal/list/create`, (req, res) => {
            ...
            fs.writeFile(
                `${moduleApi.UserAppData(req)}/checklist/lists/${listId}.json`,
                JSON.stringify({
        ...
        request.post(`/personal/list/:listId`, (req, res) => {
            ...
            if (
                !fs.existsSync(path.resolve(`${moduleApi.UserAppData(req)}/checklist/lists/${req.params.listId}.json`))
            ) {
                return res.json({ error: true });
            }

            fs.writeFile(
                path.resolve(`${moduleApi.UserAppData(req)}/checklist/lists/${req.params.listId}.json`),
        ...
        request.get(`/personal/list/:listId/create/task`, (req, res) => {
            if (
                ...
                log(
                    `no such file: ${path.resolve(
                        `${moduleApi.UserAppData(req)}/checklist/lists/${req.params.listId}.json`
                    )}`
                );
                return res.json({ error: true });
            }
            ...
            fs.readFile(
                ...
                (err, data) => {
                    if (err) {
                        log(
                            `unable to read file: ${path.resolve(
                                `${moduleApi.UserAppData(req)}/checklist/lists/${req.params.listId}.json`
                            )}`
                        );
                        return res.json({ error: true });
                    ...
                    fs.writeFile(
                        ...
                        (err) => {
                            if (err) {
                                log(
                                    `unable to write to file: ${path.resolve(
                                        `${moduleApi.UserAppData(req)}/checklist/lists/${req.params.listId}.json`
                                    )}`
                                );
                                return res.json({ error: true });
        ...
        request.post(`/personal/list/:listId/task/:taskId`, (req, res) => {
            ...
            fs.readFile(path.resolve(`${moduleApi.UserAppData(req)}/checklist/lists/${listId}.json`), (err, data) => {
                ...
                fs.writeFile(
                    path.resolve(`${moduleApi.UserAppData(req)}/checklist/lists/${listId}.json`),
                    JSON.stringify(json),
                    (err) => {
                        if (err) {
        ...
        request.get(`/assignee/:userName`, (req, res) => {
            ...
            fs.readFile(
                path.resolve(`${moduleApi.FsOrigin}/data/users/${req.params.userName}/user.json`),
                (err, data) => {
                    if (err) {
