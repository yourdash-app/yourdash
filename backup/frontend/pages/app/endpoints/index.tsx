        ...
        <main className={styles.root}>
            ...
            <section>
                <Chiplet.Row className={styles.urlSection}>
                    <Chiplet.Button disabled>{serverUrl}/api</Chiplet.Button>
                    <Chiplet.TextInput
                            ...
                            switch (queryMethod) {
                                case "GET":
                                    switch (queryType) {
                                        case "json":
                                            verifyAndReturnJson(
                                                SERVER.get(`/${queryModule}${queryUrl}`),
                                                (data) => {
                                ...
                                case "POST":
                                    switch (queryType) {
                                        case "json":
                                            verifyAndReturnJson(
                                                SERVER.post(`/${queryModule}${queryUrl}`, { body: queryBody }),
                                                (data) => {
                                                    setResponseDidError(false);
                                                    setResponse(JSON.stringify(data, null, 2));
                                                },
                                ...
                                case "DELETE":
                                    switch (queryType) {
                                        case "json":
                                            verifyAndReturnJson(
                                                SERVER.delete(`/${queryModule}${queryUrl}`),
                                                (data) => {
                                                    setResponseDidError(false);
                                                    setResponse(JSON.stringify(data, null, 2));
                                                },
                ...
                <Chiplet.Card>
                    {responseDidError && (
                        ...
                        >
                            This request received an error
                        </h1>
                    )}
                    <pre
                        style={{
