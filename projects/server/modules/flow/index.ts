...
import FlowNode, { FlowNodeShapes, FlowNodeTypes } from "./types/FlowNode.js";
...
const module: YourDashModule = {
    load(request, api) {
        request.post(`/project/create`, (req, res) => {
            ...
            fs.writeFile(
                path.resolve(`${api.UserAppData(req)}/flow/flows/${name}.flow`),
                JSON.stringify({ name: name, nodes: defaultNodes } as { name: string; nodes: FlowNode[] }),
                (err) => {
